...
The product native suite also closes the packaged app, commits a real fixture trim through the production draft transaction store, and reopens the visible project. It verifies the shortened project duration and seek range, maps output time zero to the retained source offset, then opens the same Source library item at source time zero. This proves packaged reopen and project-frame mapping for the supported single-clip reducer; live event delivery from an authenticated Codex tool remains a separate P2 gate.
