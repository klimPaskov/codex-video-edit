...
| Shared draft transaction foundation | 08, 10, 17, 30, 31; draft snapshot/transaction/checkpoint schemas and examples; guarded-tool contract | P2-05 | FFmpeg-backed tests cover trusted manual/Codex/Magic Wand origins, exact sequence/hash freshness, one bounded trim reducer, active-project summary/trim/newest-undo tools, atomic journal replay, idempotence, deterministic undo, checkpoint scope/recovery, source and baseline immutability, navigation serialization, same-head conflict, forbidden tool input and redacted errors; stdio MCP/native consumption remains incomplete |
