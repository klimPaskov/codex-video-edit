...
| Shared draft transaction and owned MCP foundation | 08, 10, 17, 21, 30, 31; draft snapshot/transaction/checkpoint schemas and examples; guarded-tool contract | P2-05 | FFmpeg-backed tests cover trusted manual/Codex/Magic Wand origins, exact sequence/hash freshness, one bounded trim reducer, active-project summary/trim/newest-undo tools, atomic journal replay, idempotence, deterministic undo, checkpoint scope/recovery, source and baseline immutability, navigation serialization, same-head conflict, forbidden tool input and redacted errors; project-runtime and renderer-reconciliation tests additionally cover atomic project/draft views, head-bound frame decode, stale-pixel rejection and mutation settlement refresh; a packaged precommitted-trim run verifies reopen, project duration/frame mapping and unchanged source preview/baseline; direct child/broker tests cover authenticated forwarding, allowlist, bounds, errors and packaged tamper; signed-out app-server verifies exact owned inventory; authenticated model invocation remains incomplete |
...
| Five-stage navigation and authorized live draft editing | 02, 04, 05, 08, 30, 46; ADR 0003 | P0-05, P1-03, P2-05 | tests/foundation/test_workflow_contracts.py contract checks; packaged native stage persistence and committed bounded-trim preview checks pass; authenticated live-edit checks remain required |
