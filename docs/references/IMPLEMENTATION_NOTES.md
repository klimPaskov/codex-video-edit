...
The partial Edit surface places playhead-based Trim start, Trim end and Undo beside the existing preview controls only in Edit. Keep them disabled at invalid boundaries and while a save is pending. This compact control is an implemented step toward the fuller referenced editor, not a reason to add empty tracks, transcript panels or decorative tool rails before they work.
