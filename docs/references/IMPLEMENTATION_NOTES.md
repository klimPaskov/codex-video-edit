...
The partial Edit surface places playhead-based Trim start, Trim end, Split, Mark/Cut controls, exact missing source-range Restore, Undo and Redo beside the existing preview controls only in Edit. Restore accepts one explicit half-open source-time interval and leaves source inventory immutable. Keep every control disabled for invalid intent or while a save is pending. This compact control is an implemented step toward the fuller referenced editor, not a reason to add empty tracks, transcript panels or decorative tool rails before they work.
