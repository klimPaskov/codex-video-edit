...
The project preview shows current and total output time. Its frame and seek bounds come from the current committed draft head. A committed trim refreshes duration and ordered clip mapping, remaps output time through the retained immutable source, and discards stale decoded pixels. The Edit stage offers compact Trim start, Trim end and Undo actions at the playhead for the selected current clip; invalid boundary actions are disabled, save failure stays actionable, and a shared-history undo may reverse a manual or assistant edit. Source library inspection remains clearly labelled as source position. Tagged 8-bit H.264/BT.709 imports can show converted display frames at native dimensions, with source seek positions resolved against presentation PTS instead of average frame rate. The two-source path crosses its join, but remains still-frame inspection without audio playback or master rendering.
