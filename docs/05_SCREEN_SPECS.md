...
The project preview shows current and total output time. Its frame and seek bounds come from the current committed draft head. A committed trim or split refreshes the ordered fragment map, remaps output time through the retained immutable source, and discards stale decoded pixels; split preserves total duration. The Edit stage offers compact Trim start, Trim end, Split and Undo actions at the playhead for the current fragment. Trim and Split require an interior playhead position and are disabled at boundaries or during a pending save; save failure stays actionable, and shared-history Undo may reverse a manual or assistant edit. Source library inspection remains clearly labelled as source position. Tagged 8-bit H.264/BT.709 imports can show converted display frames at native dimensions, with source seek positions resolved against presentation PTS instead of average frame rate. The two-source path crosses its join, but remains still-frame inspection without audio playback or master rendering.
