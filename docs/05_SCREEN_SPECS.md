...
Packaged Linux signed-out settings, login initiation/cancellation, reopen and 200% scale checks passed, with guest-only visual inspection at 100% and 200%; that earlier test suppressed external browser launching. A separate browser-equipped isolated guest opened the real OpenAI login page from packaged Settings, then cancelled to signed out. Guest-only visual inspection showed Chromium over the native editor; no host browser or input was used. The project drawer, production thread lifecycle and owned MCP inventory are implemented. A separate signed-in packaged Settings run matched the returned account plan, selected runtime model/reasoning, usage/reset values and skills across restart with guest-only input. Bounded authenticated fixture turns passed trim/undo, history reopen and Stop. A packaged synthetic parent spawned one native child that completed the two approved read tools under the inherited read-only policy; private rollout, App Server child identity/history and the native window were checked. Managed browser sign-in completion remains unverified. Project-level model defaults and renderer-selected per-turn overrides are not implemented. Preserve the modal focus/scale/one-panel rules while testing these surfaces.
