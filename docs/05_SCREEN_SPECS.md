...
Only one settings section opens at a time. Codex and the API providers section (with OpenAI API and DeepSeek choices) are implemented; packaged tests cover the disconnected/rejected-key state and authenticated OpenAI model discovery, while remembered-key model restoration and authenticated DeepSeek remain native acceptance gaps. A remembered model choice appears only after live catalog validation; a session-only key's choice ends with its session. Key bytes never appear in renderer state, and no provider row may imply a paid API turn already started.
