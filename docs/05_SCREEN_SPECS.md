...
A separate packaged native synthetic-transport test opens the OpenAI API conversation from an imported project. Send starts the bounded tool chain; the window shows a committed trim while that turn is running, then the same draft rejects a stale edit, accepts a shared-history undo, and reopens with original pixels. The visible provider response is test data, not authenticated model evidence. The drawer shows only the chosen provider and keeps its billing/context notice adjacent to Send.
