...
The subsequent P1 project prerequisite now adds actual project create/open/reopen and five-stage navigation to the bootstrap. Home lists Projects and Source library separately, and retained source entries have a working Create project action. Successful import creates an actual project with a baseline revision. Stage selection saves through main before changing the active control, retains source/frame position, and preserves prior state on failure. P1 is accepted in `.astra/results/P1.json`, including packaged native tests and guest-only visual inspection. Later editing, capture and export controls remain separate implementation work. The complete screen requirements below remain authoritative.
