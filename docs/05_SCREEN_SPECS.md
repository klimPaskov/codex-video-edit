...
Only one settings section opens at a time. Codex and the API providers section (with OpenAI API, DeepSeek and Gemini API choices) are implemented; packaged tests cover the disconnected/rejected-key state, authenticated OpenAI model discovery, and protected remembered-model restoration with real GNOME Secret Service. Authenticated DeepSeek and Gemini remain native acceptance gaps. A remembered model choice appears only after live catalog validation; a session-only key's choice ends with its session. Key bytes never appear in renderer state, and no provider row may imply a paid API turn already started.
