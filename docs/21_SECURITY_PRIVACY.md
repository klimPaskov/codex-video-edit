...
Feature flags are route- and model-protocol-specific on both thread start and resume. MCP-bound threads keep `code_mode_only` false, omit dynamic-only enable flags, exclude `multi_agent_v1` and `multi_agent_v2` and keep native agents disabled. Both routes disable the code-mode host's in-process fallback. Dynamic V1-capable Codex threads enable code mode and the packaged host and support V1 child collaboration at depth 1. GPT-6-Luna V2 enables the direct-only `codex_video_edit_agents` namespace with capacity two; the six V2 child functions stay out of nested code-mode, and the parent supplies only the current path-free project/timeline snapshots. Correlate the spawn call ID, server-owned child activity, parent thread and child thread before projecting generic activity. Both routes explicitly set the pinned supported unrelated browser, app, connector, plugin, shell, search, image, memory, hook, remote-control and commit flags false. Focused request tests and packaged dynamic split/V2-child fixtures passed; these bounded observations do not establish a complete upstream tool allowlist.
