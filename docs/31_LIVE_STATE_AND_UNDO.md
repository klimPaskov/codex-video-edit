...
The bounded reducer trims one edge of a committed clip or splits it at an interior playhead position. Split retains the left clip ID and derives the right ID from trusted operation authority. Ordered fragments map to immutable source intervals, and both operations store exact prior state for deterministic journal replay and newest-transaction undo. Source immutability is tested on one- and two-source drafts. Range delete/restore, general move and reorder, redo, group undo, externally concurrent writers, directory durability after power loss, and full P3/P6 recovery remain incomplete.

## Live Codex changes

During a Codex turn, valid transactions may be committed to the active draft as tools complete. The current trim, split and newest-undo slice rereads the atomic draft after every settled mutation and updates project duration, seek bounds, committed fragment map and preview source mapping from that sequence/hash. It also reconciles a transaction that committed before an uncertain tool response. The native Edit stage exposes manual trim, split and newest Undo; transcript, marker, broader history and grouped undo remain dependent on later editor surfaces. Split is currently a manual action, not an additional model-visible MCP tool.
