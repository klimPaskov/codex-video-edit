...
The shared draft reducer now supports reversible half-open ripple cuts across ordered source fragments, including source joins, and exact restoration of one missing half-open source-time interval from an immutable baseline source. It records complete before/after clip maps, replays and undoes each operation, and preserves immutable source files. The native Edit stage exposes manual in/out marks and exact source-range restore; guarded Codex and fixed API-provider tools use the same reducers. Main can record a structural-only checkpoint for the current manual group from validated timeline, journal and managed-source evidence, but this is not an editorial pass checkpoint. Transcript-driven selection, synchronized source sets, meaning checks, rendered A/V join review, omission pass, and automatic first cut remain unimplemented. This adaptation does not change P1 completion status or claim an editorial edit of the user-supplied video.

The guarded `cut.delete_ranges` primitive now accepts a confirmed, nonoverlapping descending batch of 2–16 output-time intervals and persists it as one reversible transaction. The app does not choose those ranges or mark the spoken pass verified. Synthetic and private supplied-footage native runs prove structural application, preview mapping, shared Undo and reopen; they do not prove meaning-preserving cleanup or listened A/V joins.
