...
The shared draft reducer now supports a reversible half-open ripple cut across ordered source fragments, including a source join. It records the exact prior clip map, replays and undoes the cut, and preserves immutable source files. The native Edit stage exposes manual in/out marks; guarded Codex and fixed API-provider tools can submit a direct user-specified output-time range through the same reducer. This is a structural cut foundation only: transcript-driven selection, synchronized source sets, meaning checks, rendered A/V join review, omission pass, pass checkpoint and automatic first cut remain unimplemented. This adaptation does not change P1 completion status or claim an editorial edit of the user-supplied video.
