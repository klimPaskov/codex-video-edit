...
Still-frame source seeking uses verified presentation PTS rather than an average-frame-rate guess. Packet PTS is sorted because B-frame packet order is decode order. The resulting display-only seek index never feeds canonical render or master encoding. A mixed-source timeline still needs an explicit source-to-output map and a verified final-frame boundary.
