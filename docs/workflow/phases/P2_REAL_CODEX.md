...
The partial manual Edit surface uses the same main-owned transaction service for playhead-based clip-edge trim, interior split, a marked ripple range cut and newest undo. Its IPC carries a strict committed draft head and bounded intent, while main supplies the trusted manual origin and generated journal IDs. A split keeps the left fragment identity and creates a deterministic right fragment identity from the trusted operation. A half-open range cut may cross source joins or remove a source's complete visible span; it reflows surviving fragments without deleting immutable source inventory or baseline. The projected fragment map must update after manual and assistant commits before further tool targeting. This adds no model-visible MCP tool and does not satisfy the remaining guarded-tool policy, provider access, or P2 phase gates.
