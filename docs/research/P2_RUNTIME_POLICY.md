...
The stable schemas initially generated under private test evidence omit these experimental properties. The implementation regenerated TypeScript/JSON with `--experimental` from the same pinned binary, retained the complete output privately, and pins a reviewed 207-type dependency closure by original hashes, including the consumed `ThreadTurnsListParams` and `ThreadTurnsListResponse` shapes. Request objects compile against the generated inputs, and the signed-out packaged server accepts `initialize.capabilities.experimentalApi` plus the owned MCP configuration. A signed-out startup cannot exercise `thread/start`, `thread/resume`, or `turn/start`, so live no-environment and persisted-history acceptance remain authenticated gates. Experimental compatibility is a versioned product dependency.
