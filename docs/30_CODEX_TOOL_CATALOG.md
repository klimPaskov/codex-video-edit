...
The packaged dynamic split fixture requires a real Luna/high code-mode answer with exactly seven guarded editor functions and five supported v1 collaboration functions on the dynamic route, with zero app or other nested functions, then one guarded `cut_split` commit and shared Undo. Fresh synthetic two-source turns passed with hostile Apps configuration, reopen, exact previews and immutable inputs. A mismatched model-supplied project ID earlier failed closed with no journal change; new dynamic schemas now constrain the ID to the thread's main-owned active project while main retains independent authorization. Treat the count as one-turn model-visible evidence; the supported App Server API does not provide an authoritative complete future-turn catalog.
