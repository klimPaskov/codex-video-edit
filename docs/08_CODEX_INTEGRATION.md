...
During a dynamic turn start, a validated owned `turn/started` notification can provisionally correlate a host call before the `turn/start` response arrives. The response must confirm the same turn before buffered activity reaches the renderer. Unknown, completed or contradictory turns quarantine the client before further dispatch. Focused tests cover these ordering and failure cases; a direct isolated official App Server client also completed one owned Luna/high read without MCP. That live check did not measure an early-call race or mutate a draft. Packaged main still chooses MCP, so dynamic editing, Undo and reopen in the native product remain unverified.
