...
A later packaged Luna/high dynamic-route split turn also passed after a code-mode inventory answer showing exactly six owned nested editor tools and zero unowned names in that turn. The app committed a two-source split while the turn ran, projected exact split/join frames, shared Undo and reopened the draft. Synthetic and privately staged supplied-footage runs passed with immutable source/baseline checks. This remains bounded observation rather than an authoritative permanent upstream-tool catalog.
