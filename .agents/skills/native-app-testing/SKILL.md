...
For the packaged provider draft route, run `tests/native/api-provider-draft-fixture.test.ts` in the unprivileged guest. Its deterministic main-process transport prevents any paid request; verify the actual imported project, visible trim while the turn runs, stale rejection, shared undo, provider failure, source/baseline integrity, reopen and guest-only screenshot. Retain timing failures separately and require a complete passing rerun. This does not replace live authenticated provider acceptance.
