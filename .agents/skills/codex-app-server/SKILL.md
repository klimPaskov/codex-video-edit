...
Do not infer tool-search non-use from `thread/turns/list`: the pinned `v2/ThreadItem` union omits search items even though the lower-level `ResponseItem` includes them. A missing collaboration item and child thread still show that no native child spawned. If diagnosing discovery during a live turn, classify only the official raw search completion notification in private evidence; never expose raw payloads through renderer IPC or claim a complete model-visible catalog from request flags or model metadata.
