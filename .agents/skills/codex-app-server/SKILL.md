...
initialize and derive one reviewed feature policy from the stored route on both start and
resume. Keep MCP-bound threads in the tested no-code-mode/no-native-agent path. On the
dynamic Codex route, allow only the supported v1 child path at depth 1; main must correlate
the parent spawn and active child turn, then permit children to read only the project and
timeline summaries. Disable the confirmed unrelated default-mode-input, permission,
...
Account type alone does not prove managed-token provenance. Restrict the actual login entrypoint and account-directory ownership. Before enabling turns, verify the pinned no-environment/tool policy and route-specific guarded editor boundary. Keep native collaboration disabled for MCP-bound project threads. The dynamic Codex subscription route may enable only native v1 children at depth 1; the main client must correlate a server-owned parent spawn, child thread, and active child turn, then allow only the two read-only project/timeline summary tools. Preserve empty child environments, read-only/no-network policy, and approval rejection. A single packaged passing child read proves that tested path only, not a complete upstream tool catalog. Bootstrap stdio tests cannot replace native sign-in and authenticated reversible-edit evidence.
