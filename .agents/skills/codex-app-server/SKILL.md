...
The optional internal host-call adapter now uses pinned generated request/response types, maps only the reviewed six tool schemas into one namespace, bounds arguments/results, and allows only configured owned calls and redacted stream/history items. Product main still does not supply the invoker or register dynamic tools. Before enabling it, cover turn-start races beyond one direct observation, select MCP or dynamic processes from each stored project route, and prove the existing shared transaction/Undo/reopen path in packaged Electron.
