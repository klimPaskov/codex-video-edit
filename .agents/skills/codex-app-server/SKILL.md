...
The optional internal host-call adapter uses pinned generated request/response types, maps only the reviewed six tool schemas into one namespace, bounds arguments/results, and allows only configured owned calls and redacted stream/history items. A validated owned `turn/started` notification may provisionally correlate a call before the RPC response; buffered activity stays hidden until the response confirms the same ID. Reject missing, completed or contradictory turns. Product main still does not supply the invoker or register dynamic tools. Before enabling it, verify live early-call behavior, select MCP or dynamic processes from each stored project route, and prove shared transaction/Undo/reopen in packaged Electron.
...
The project-thread registry now writes route-tagged v2 bindings. Decode every strict v1 binding as MCP without mutating its file on read; only a later serialized write migrates it. A new internal dynamic thread sends exactly six reviewed definitions at start, while resume follows the stored route and cannot silently switch an MCP thread. Reject mixed or incompatible process routing before an App Server RPC, and reject cross-route stream items or server calls. Packaged main still uses MCP; require isolated native migration and guarded edit/Undo/reopen before activation.

An isolated packaged Luna/high MCP regression passed trim, same-thread reopen, Codex Undo, interruption and guest-only window inspection. A separate synthetic v1 registry file for that real thread reopened and completed a read-only guarded turn without a write; another product-created binding migrated the file to v2 and preserved its old ID. A second broad edit harness failed a post-turn assertion after a correct trim, so count only the migration observations from it. Host-defined guarded draft commit/Undo/reopen is still required.
