...
The route-aware native split fixture can require `--require-dynamic --probe-surface --require-luna` to check the model-visible code-mode inventory and a guarded split in one packaged turn. It passed with exactly six owned and zero unowned nested tools, then committed split/Undo/reopen on synthetic and privately staged supplied footage. Keep this bounded result separate from a permanent complete upstream-tool allowlist and from native-child inheritance.
