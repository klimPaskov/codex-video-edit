...
Thread feature flags are selected from the persisted route on both start and resume. MCP must keep code_mode_only false, omit dynamic-only code-mode enable fields, exclude multi_agent_v1, and disable native agents. Dynamic Codex may enable code mode and the packaged host only with in-process fallback disabled; only the supported v1 child path at depth 1 is permitted, with main-owned spawn/turn correlation and read-only summaries. Set supported unrelated browser, app, connector, plugin, shell, search, image, memory, hook, remote-control and commit features false on both routes. Also disable request-permission and rule tools, MCP OAuth elicitation and dependency installation, API-key model discovery, passive Chronicle screen memory, external-agent memory import, persisted goals, and plugin suggestions. Do not disable the supported v1 child route as a blanket response to an incomplete upstream catalog. Exact protocol/runtime tests and a packaged synthetic dynamic split fixture passed. These flags and the tested inventory do not establish a complete effective upstream tool catalog.
