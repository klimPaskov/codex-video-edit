...
- preflight every required packaged renderer/preload resource; missing JS/CSS must not leave an inert window
- verify explicit startup scale overrides restored Chromium origin zoom, including read-failure defaults
- test the real native renderer recovery dialog, one-attempt limit and intentional-shutdown guard; preserve original build copies and source/project data
