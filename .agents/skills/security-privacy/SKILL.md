...
The internal optional dynamic adapter rejects unreviewed namespace/name, extra fields, oversized arguments and stale known turns before invoking an app-owned callback. Only fixed safe errors or bounded results reach the model; the renderer projection omits arguments and results. It is not active in packaged main. Do not enable it while turn-start correlation is ambiguous or existing MCP-backed project threads would be silently rerouted.

The main-owned registry now records an exact MCP or dynamic tool route in v2. Treat v1 as MCP, leave it unchanged on read, and reject unknown or conflicting routes without replacing evidence. A dynamic thread must neither project MCP items nor accept an MCP-only process on resume; an MCP thread must reject host calls even when an internal invoker is present. Continue to require packaged native compatibility proof before switching product routing.
