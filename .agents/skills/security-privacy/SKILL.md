...
For a dynamic-route model-visible inventory claim, require a bounded real Luna/high code-mode turn with exact six owned nested names and zero unowned names, followed by an owned guarded edit and source-integrity check in the same packaged app. The synthetic and private supplied two-source split fixtures passed this bounded test. Do not infer that an outer tool or every future upstream tool is unavailable, or enable native children from this observation alone.
