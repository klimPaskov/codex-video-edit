...
The generated protocol types under `packages/codex-bridge/src/generated/` derive from official Codex 0.155.1 (Copyright 2025 OpenAI, Apache-2.0). The pinned upstream licence is retained at `licenses/CODEX-APACHE-2.0.txt`. Generation markers remain intact; only explicit TypeScript import extensions and formatting are normalized by the reviewed sync script. The type-generation manifest records original source hashes. The P2 native build bundles the official native runtime, its same-version code-mode host and this Apache-2.0 licence under resources/codex outside ASAR, with a generated version/hash manifest for both executables. These builds remain private test artifacts; no binary or credential is checked into Git or publicly released by source publication. Installer and release acceptance remain separate.
