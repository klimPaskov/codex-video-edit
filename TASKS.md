...
- [ ] P2-07 Reject fake responses and stale protocol assumptions. Distinguish offline guarded-service freshness on a real Codex edit from live stdio error relay, bounded native non-use of a forbidden command from complete tool unavailability, and model subagent claims from a completed spawn and child read. The current 0.155.1 Luna path explicitly disables the supported plan, user-input and native-agent controls at App Server launch and thread create/resume, and a bounded native nested-tool probe passed. Separate direct App Server probes passed one namespaced host-defined tool with no MCP server, an owned-only bounded code-mode inventory, and one process-restart/thread-resume call. This is not production guarded-tool routing, old MCP-thread migration, or complete effective confinement. Safe native-child inheritance remains open.
  Partial boundary note: an optional internal boundary now validates and projects only six reviewed host-defined tool names in unit tests. Packaged main still uses MCP. Starting-turn correlation, legacy MCP-thread migration, native guarded draft dispatch and complete effective confinement remain open.
