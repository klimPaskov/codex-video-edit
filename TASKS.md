...
- [x] P2-01 Spawn the official Codex app-server over stdio and implement initialize lifecycle and restart recovery.
  Acceptance evidence: transport handshake/restart tests and authenticated packaged Luna/high Electron runs verified initialization, idle child recovery, and child loss after a guarded end-trim committed while its turn was still running. Settings Reconnect started a new pinned child, restored the same dynamic project thread, and produced no duplicate committed transaction; shared manual Undo restored the fixture. Source and baseline remained unchanged. The remaining Codex sign-in, discovery, thread-policy and API-provider gates, along with full P2 acceptance, remain open.
- [ ] P2-02 Add ChatGPT-managed browser and optional device-code login, logout, account state, and rate-limit display. Keep the one-time device code in the explicit Settings attempt only; require an actual completion callback and reconciled signed-in account before claiming managed login completion.
  Partial browser-flow evidence: a packaged Electron test against the real pinned App Server rejected a mismatched callback state, processed a matching `access_denied`, returned to signed out with fixed recovery text, redacted provider detail, and allowed retry/cancel. A packaged signed-in Settings regression now matches plan/model/skills/usage, explicitly logs out, and verifies signed-out state and empty models/usage after reopen. Device-code completion and signed-in reopen are separately recorded in `docs/workflow/progress/P2.md`; successful browser OAuth completion remains unverified, so this task stays open.
- [x] P2-03 Discover models, reasoning options, skills, and skill changes at runtime. A fresh ChatGPT preference defaults to live-catalog GPT-6-Luna/high under ADR 0016; preserve explicit choices and show a recovery state when that pair is unavailable.
  Acceptance evidence: unit tests cover runtime model/reasoning decode, Luna/high selection when listed, preservation of a valid explicit choice, an actionable unavailable-pair state, and immediate refresh after `skills/changed`. The packaged `codex-luna-default.test.ts` passed twice with a fresh signed-in profile against live 0.155.1 `model/list`; both launches offered and selected GPT-6-Luna/high. A packaged Settings run added a local fixture skill while connected and verified it appeared in the live list without restarting. P2 remains open.
- [x] P2-04 Implement durable project threads, streaming items, interrupt, retry, and compact user-facing activity. Filter native child notifications from the parent projection and require correlated server history before claiming a subagent ran. A retry control is available only after an authoritative failed terminal state; it resends the most recent user request after explicit user action. Interrupted or uncertain turns do not offer automatic replay.
  Acceptance evidence: stream/protocol/client/controller tests cover ordered live items, bounded history restoration, redaction, terminal failure/interruption, retry through a fresh message identity, and native child-event filtering. The signed-in packaged Luna/high fixture passed a real guarded edit, same-thread/history reopen, live committed-state projection, real Stop during a read-only turn, and unchanged journal after Stop. Separate V1/V2 packaged child runs required correlated server-owned spawn and child history before read-only child work. The new retry path passed strict view/IPC validation and controller tests; packaged Electron smoke and guest inspection confirmed the control stays hidden without a failed turn. A live failed model turn and enabled-state screenshot were not run. This task is complete; remaining P2 tasks and the phase remain open.
- [ ] P2-05 Implement the guarded codex-video-edit MCP tool server and shared validated transaction engine, including expected draft sequence, durable journal, atomic persistence, deterministic inverse/undo, redo-stack replay, and committed-transaction recovery required by the real fixture edit (ADR 0013). Partial native Edit controls now route playhead trim, split, a marked ripple range cut, exact missing source-range restore, newest undo and stacked redo through this same journal, with committed clip-map refresh. Packaged synthetic Electron verifies manual Redo through IPC before and after project reopen, with exact decoded preview and immutable-input checks. The guarded Codex and fixed API-provider surfaces also expose reversible `cut.split` and bounded `cut.delete_range` over that reducer; the earlier seven-tool dynamic-route inventory, stale/invalid-range rejection, real Codex cross-source native preview and shared Undo passed on synthetic and supplied footage. Real Codex split passed on synthetic and the supplied two-source footage with live fragment projection, exact frames, Undo and reopen; the latter private run used Luna/high on the current official runtime after the fixture passed. Complete guarded-tool policy and editorial pass groups, current sequence/hash preconditions and truthful verified checkpoints in docs/47_EDITORIAL_FIRST_CUT.md. A passing checkpoint check must include at least one evidence ID; checkpoint persistence does not itself perform editorial or A/V review.
  ...
  - A pass checkpoint now rejects any passing check with an empty evidence list in both the domain validator and published schema; it remains an engine contract and does not perform or claim a spoken/A/V review.
  - Manual Edit exposes a typed source-range restore request containing only source identity, exact half-open source times, and the committed draft head. Main requires the active project in Edit, resolves the immutable baseline source and injects manual authority into the shared journal. Packaged two-source native tests cover stage gating, visible-overlap rejection, cut-then-restore, preview mapping, Undo/Redo after reopen, and source/baseline integrity; transcript-driven restoration remains open.
  - The main-only structural checkpoint helper derives its checks from validated current timeline, journal, and managed-source reads; main selects the current manual-origin group and rejects AI-claimed manual/semantic groups. The scoped Review IPC accepts only the active project identity and cannot supply group, status, or evidence; no AI tool reaches this path. Its status and summary exclude editorial/A/V review.
  - The Review stage exposes a typed Check draft integrity action that re-reads the active committed snapshot in main and returns only a path-free view. When the newest applied transaction is main-owned manual work, main may persist a structural-only checkpoint with concrete evidence IDs; AI-origin work is ineligible. Packaged native tests cover checkpoint persistence, corrupted-managed-source failure/recovery, unchanged project/baseline/original-source bytes, and Review-only visibility. It does not create an editorial checkpoint or claim media QA.
- [x] P2-06 Prove a real authenticated Codex turn can inspect a fixture project and apply a draft-only edit. Bootstrap read tools on create/resume with the main-owned validated project identity; obtain current draft identifiers, sequence and hash through those tools before mutation.
  Acceptance evidence: a fresh-profile packaged Linux Electron run with real GPT-6-Luna/high on two synthetic sources completed one guarded range-batch edit. The exact completed App Server history contained successful correlated project/timeline outputs before the mutation; the project ID, draft ID, base revision, sequence, and timeline hash from both read results matched each other and the edit's freshness fields. The edit committed while the turn ran, projected to the native preview, shared Undo, and reopen restored the original exact frame; source and baseline bytes stayed unchanged. Eight new dynamic tool schemas bind `project_id` to the main-owned project; unit coverage checks all eight, thread start/resume route behavior, and independent main-side project/freshness checks. Separate packaged dynamic-thread trim/Undo/reopen, read-only interruption, and live/offline stale sequence/hash rejection remain verified. This accepts the bounded real-turn edit gate only; meaning-preserving speech selection, complete guarded-tool confinement, A/V review, Magic Wand, API-provider edits, and export remain open.
...
Current bounded evidence (2026-09-26; this phase remains incomplete while model, provider, tool-policy, media review, and export gates remain open):
...
The earlier clean isolated Docker workspace passed npm run check: 42 foundation tests, 315 media tests, 43 schema/example pairs, and delivery validation with 20 reference images; the native GPT-6 restore/split and V1 child fixtures passed. Account files, media, raw rollout, screenshots, and result JSON remained in the guest. The authenticated read-before-edit acceptance was subsequently recorded in `docs/workflow/progress/P2.md`. No P2 phase result is written.
...
- [ ] P6-01 Implement select, split, trim, ripple delete, restore, move, snap, undo, and redo. Partial native Edit controls now trim or split the current clip at the playhead, mark an output-time range for ripple deletion across fragments or sources, restore one exact missing source interval, and use newest Undo plus stack-ordered Redo from the durable journal. Packaged synthetic Electron verifies Redo before and after reopen with exact preview and immutable-input checks; transcript-linked restore, broader selection, move, snap, and full A/V edit flows remain open.
