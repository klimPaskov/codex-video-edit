...
- [ ] P2-04 Implement durable project threads, streaming items, interrupt, retry, and compact user-facing activity. Filter native child notifications from the parent projection and require correlated server history before claiming a subagent ran. A retry control is available only after an authoritative failed terminal state; it resends the most recent user request after explicit user action. Interrupted or uncertain turns do not offer automatic replay.
