...
import { assertCodexSelection } from "../../../packages/domain/src/codex-view.ts";
import type { CodexSelection } from "../../../packages/domain/src/codex-view.ts";

const pending = new Map<string, Promise<unknown>>();
const loadError =
  "Codex model settings could not be loaded. Check local storage access and restart the application.";
const saveError =
  "Codex model settings could not be saved. Check local storage access and try again.";
...
/** Main-owned userData directory only. Model settings never contain media or paths. */
export class CodexSettingsStore {
  ...
  private async current(): Promise<CodexSelection | null> {
    ...
    try {
      ...
      return { modelId: value.modelId, reasoning: value.reasoning };
    } finally {
      await handle.close();
    }
  }
  read(): Promise<CodexSelection | null> {
  ...
  write(value: unknown): Promise<CodexSelection | null> {
    ...
    return this.serialize(async () => {
      ...
      try {
        ...
        await this.current();
        staged = join(this.root, `.selection-${randomUUID()}.tmp`);
        ...
        await this.current();
        await rename(staged, join(this.root, "selection.json"));
        staged = undefined;
        return { ...snapshot };
      } catch {
